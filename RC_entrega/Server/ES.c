...
void handle_tcp(int tcp_fd) {
    ...
    if (strncmp(cmd, "CRE", 3) == 0) {
        ...
        while (eid <= 999) {
            ...
            snprintf(path, sizeof(path), "Server/EVENTS/%s", eid_str);
        ...
        snprintf(path_desc, sizeof(path_desc), "Server/EVENTS/%s/DESCRIPTION", eid_str);
        snprintf(path_resv, sizeof(path_resv), "Server/EVENTS/%s/RESERVATIONS", eid_str);
        ...
        snprintf(path, sizeof(path), "Server/EVENTS/%s/START_%s.txt", eid_str, eid_str);
