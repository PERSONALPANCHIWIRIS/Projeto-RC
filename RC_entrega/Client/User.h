...
public:
    ...
    string show_cmd_tcp(string message){
        ...
        string filename_dir = "Client/SHOW/" + eid + "/" + fname;

        //Verifica se a pasta SHOW existe, se não existir cria-a
        if (!dir_exists(("Client/SHOW/" + eid).c_str()) || !dir_exists("Client/SHOW/")) {
            mkdir ("Client/SHOW", 0777);
            mkdir(("Client/SHOW/" + eid).c_str(), 0777);
        }
